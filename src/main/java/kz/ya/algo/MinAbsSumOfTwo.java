...
public class MinAbsSumOfTwo {
    ...
    public static void main(String[] args) {
        int[] A1 = {1, 4, -3};
        System.out.println(getMinAbsSumOfTwo(A1));
        
        int[] A2 = {-8, 4, 5, -10, 3};
        System.out.println(getMinAbsSumOfTwo(A2));
    }

    public static int getMinAbsSumOfTwo(int[] A) {
        Arrays.sort(A); // Dual-Pivot Quicksort, O(n log(n))
        int top = A.length - 1, btm = 0, sum;
        int minAbsSum = Math.abs(A[top] + A[btm]);
        while (top >= btm) {
            sum = A[top] + A[btm];
            if (sum <= 0) {
                btm++;
